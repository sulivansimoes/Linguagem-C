...
Calcule e imprima a média semestral. O programa só deverá aceitar notas válidas (uma nota válida deve pertencer
...
informe um código (1 ou 2) indicando se ele deseja ou não executar o algoritmo novamente, (aceitar apenas os código 1 ou 2). 
...
lidas duas notas válidas, deve ser lido um valor inteiro X . O programa deve parar quando o valor 
...
deve ser apresentada novamente se o valor da entrada padrão para X for menor do que 1 ou maior do que 2, 
...
int main(void)
{
	...
	do{
		...
		media = 0; 	 //zera media antes de entrar no laço novamente
		printf("novo calculo (1-sim 2-nao)\n");
		scanf("%d",&tst);
		
		while(tst!=1 && tst!=2) //valida a verificação para laço laço principal
		{
			printf("novo calculo (1-sim 2-nao)\n");	
			scanf("%d",&tst);
		}		
	}while(tst!=2); // Se tst igual a 1 continua no laço principal
